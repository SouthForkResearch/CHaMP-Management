...
# install.packages(c('dplyr', 'tidyr', 'gdata', 'ggplot2', 'gridExtra', 'RColorBrewer'))
...
# setwd('/Users/SFR-17/Documents/RWorkspace/CHaMP_QA_2017')
setwd('/Users/kevin/Dropbox/ISEMP/Analysis_Projects/CHaMP_metrics/QA_Test_20171222')

# folder where data from CM.org is stored
cm.dir = '/Users/kevin/Dropbox/ISEMP/Analysis_Projects/CHaMP_metrics/QA_Test_20171222/'
# cm.dir = '/Users/SFR-17/Documents/RWorkspace/CHaMP_QA_2017/'
# name of file with CHaMP data
# cm.name = 'CHaMPQA_2011_2014_20150211.xlsx'
cm.name = 'MetricVisitInformation_20171222.csv'

# Name of data file for the MetricsDataDictionary.csv
# dictionary.name = "MetricsDataDictionary20150121.xlsx"
dictionary.name = "MetricsDataDictionary-2014_20171221.xlsx"
# path where dictionary is stored
dictionary.dir = '/Users/kevin/Dropbox/ISEMP/Analysis_Projects/CHaMP_metrics/QA_Test_20171222/'
# dictionary.dir = '/Users/SFR-17/Documents/RWorkspace/CHaMP_QA_2017/'
...
AnnualPlot = function(met, my.data) {
  ...
  comp.data = my.data %>%
    ...
    select(SiteName, Year, Metric = one_of(met)) %>%
    filter(!is.na(Metric)) %>%
    tidyr::spread(Year, Metric)
...
WatershedAnnualPlot = function(met, my.data, wtsd) {
  ...
  plot.data = droplevels(subset(my.data, Watershed==wtsd))
  ...
  p2 = ggplot(plot.data, aes(x=SiteName, y=plot.data[,met])) +
    ...
    scale_y_continuous(limits=range(plot.data[,met], na.rm=T)*c(1, 1.05)) +
    stat_summary(fun.data=give.n, geom='text', vjust=1.5) +
    guides(fill=guide_legend(ncol=min(4, nlevels(plot.data$Category)))) +
    labs(x='Site', y=with(data.dictionary, DisplayName[match(met, DisplayNameShort)]), title=wtsd)

  return(list(VC.plot = p,
              Strata.plot = p2,
              plot.data=plot.data))
}
...
RepeatVisitPlot = function(met, my.data) {
  comp.data = my.data %>%
    ...
    select(Year, SiteYr, VisitNumber, matches(met)) %>%
    tidyr::spread(VisitNumber, met) %>%
    dplyr::rename(Visit.1 = `1`,
                  Visit.2 = `2`)

  p = ggplot(comp.data, aes(x=Visit.1, y=Visit.2)) +
    # geom_point(aes(shape=as.factor(Year), color=ValleyClass)) +
    geom_point(aes(shape=as.factor(Year))) +
    geom_abline(intercept=0, slope=1, lty=2, col='darkgray') +
    theme_bw() +
    scale_shape_manual(name='Year', values=c(1, 17, 8, 10)[1:nlevels(as.factor(comp.data$Year))]) +
    scale_color_brewer(name='Valley Class', palette='Set1') +
...
WithinYearPlot = function(met, plot.data, boxplot.catg = 'Watershed', boxplot.title='By Watershed', main.title=NULL) {
  ...
  p2 = ggplot(plot.data, aes(x=plot.data[,boxplot.catg], y= plot.data[,met])) +
  ...
  p = suppressWarnings(grid.arrange(grobs = list(p1, p2, p3, p4),
                                    ncol = 2,
                                    nrow = 2,
                                    top=main.title))
...
data.dictionary = read_excel(paste0(dictionary.dir, dictionary.name))
...
metrics$SiteYr = with(metrics, paste(SiteName, Year, sep='.'))
# put in reasonable order
metrics = metrics[with(metrics, order(SiteName, Year, as.Date(SampleDate, format='%m/%d/%y'))),]

# add VisitNumber
metrics = metrics %>%
  group_by(SiteYr) %>%
  dplyr::mutate(VisitNumber = 1:n()) %>%
  ungroup()
...
core.data = droplevels(subset(metrics, CHaMP.Core == 'Yes' & VisitObjective=='Primary Visit'))

# Annual panel data
annual.data = droplevels(subset(metrics, Panel == 'Annual' & VisitObjective=='Primary Visit'))

# 10% repeat visits
rep.data = droplevels(subset(metrics, CHaMP.10..Revisit =='Yes'))

# data for variance decomposition
var.decomp.data = droplevels(subset(metrics, VisitStatus=='Released to Public' & Watershed %in% unique(Watershed[CHaMP.Core=='Yes'])))

# core.wtsds = c('Entiat', 'Wenatchee', 'Methow', 'John Day', 'Tucannon', 'Lemhi', 'South Fork Salmon', 'Upper Grande Ronde')
# var.decomp.data = droplevels(subset(metrics, VisitStatus=='Released to Public' & Watershed %in% core.wtsds))
...
for(met in met.nms) {
  ...
  if(sum(table(annual.data$Year, !is.na(annual.data[,met]))[,'TRUE']!=0)<=1) {
  	which.yrs = unique(annual.data$Year)[which(table(annual.data$Year, !is.na(annual.data[,met]))[,'TRUE']==0)]
...
for(wtsd in levels(annual.data$Watershed)) {
  cat(paste('Making plots for', wtsd, '\n'))
...
#----------------------------------------------------------------
# 10% repeat visits
for(met in met.nms) ggsave(filename=paste0(met, '_CHaMP_revists.png'),
                           plot=RepeatVisitPlot(met, rep.data),
                           path=qa.rep,
                           width=7,
                           height=7,
                           units='in',
...
#----------------------------------------------------------------
for(yr in unique(metrics$Year)) {
  cat(paste('Plotting year', yr, '\n'))

  yr.data = as.data.frame(droplevels(subset(metrics, Year==yr)))
  ...
  for(met in met.nms) {
    ...
    my.plot = WithinYearPlot(met, yr.data, boxplot.catg = 'Watershed', main.title=paste('All Watersheds:', with(data.dictionary, DisplayName[match(met, DisplayNameShort)])))
    ggsave(filename=paste0(met, 'CHaMP.', yr, '.png'), plot=my.plot, path=paste0(qa.1yr, '/', yr, '/AllChamp/'), width=13, height=7, units='in', dpi=75)

    rm(my.plot)
  }

  for(wtsd in unique(yr.data$Watershed)) {
    cat(paste('\t Plotting', wtsd, '\n'))

    wtsd.data = subset(yr.data, Watershed==wtsd)
    ...
    for(met in met.nms) {
      ...
      my.plot = WithinYearPlot(met, wtsd.data, boxplot.catg = 'Category', boxplot.title='By Strata', main.title=paste0(wtsd, ': ', with(data.dictionary, DisplayName[match(met, DisplayNameShort)])))
...
for(i in 1:nrow(met.vs.met.tab)) {
  ...
  cor.df = metrics %>%
    select(SiteName,
           Year,
           Metric.1 = as.character(met.vs.met.tab[i,1]),
           Metric.2 = as.character(met.vs.met.tab[i,2])) %>%
    mutate(Year = factor(Year,
                         levels = c(sort(unique(metrics$Year)), 'All')))

  cor.tab = cor.df %>%
    group_by(Year) %>%
    summarise(r = cor(Metric.1, Metric.2,
  ...
  p = ggplot(cor.df,
    ...
    geom_point(aes(#shape = Year,
      color = Year)) +
    theme_bw() +
    # scale_shape_discrete(name='Year               r',
    #                      labels=with(cor.tab[-nrow(cor.tab),], paste(Year, round(r, 2), sep='    '))) +
    scale_color_brewer(name='Year          r',
                       labels=with(cor.tab[-nrow(cor.tab),], paste(Year, round(r, 2), sep='    ')),
                       palette='Set1') +
    annotate(geom = 'text',
             label=paste('r =', round(cor.tab$r[nrow(cor.tab)], 2)),
             x = -Inf,
             y = Inf,
             hjust = -0.1,
             vjust = 1.5,
             size = 7) +
    labs(title = paste(met.vs.met.tab[i,1], 'vs.\n', met.vs.met.tab[i,2]),
         x = as.character(met.vs.met.tab[i,1]),
         y = as.character(met.vs.met.tab[i,2]))


  ggsave(filename=paste0(met.vs.met.tab[i,1], '_vs_', met.vs.met.tab[i,2], '.png'),
         plot=p,
         path=qa.add,
         width=7, height=7, units='in', dpi=75)
...
met.melt = core.data %>%
  select(SiteName, Year, Watershed, ValleyClass, Category, one_of(met.nms)) %>%
  gather(Metric, value, one_of(met.nms))
...
wtsd.yr = met.melt %>%
  group_by(Metric, Watershed, Year) %>%
...
vc.yr = met.melt %>%
  group_by(Metric, ValleyClass, Year) %>%
  summarise(n = length(value),
            not.NA = sum(!is.na(value)),
            is.NA = sum(is.na(value)),
            Mean = mean(value, na.rm=T),
...
strata.yr = met.melt %>%
  group_by(Metric, Watershed, Category, Year) %>%
  summarise(n = length(value),
            not.NA = sum(!is.na(value)),
            is.NA = sum(is.na(value)),
            Mean = mean(value, na.rm=T),
...
strata.all = met.melt %>%
  group_by(Metric, Watershed, Category) %>%
  summarise(Year = 'All',
            n = length(value),
            not.NA = sum(!is.na(value)),
            is.NA = sum(is.na(value)),
...
strat.2011 = ddply(subset(frames, Target2011=='Target'), .(CHaMPshed, Strata2011), summarize,
                   Year = 2011,
                   Length = sum(LengthKM))
names(strat.2011) = c('Watershed', 'Category', 'Year', 'Strata.Length')
strat.2012 = ddply(subset(frames, Target2012=='Target'), .(CHaMPshed, Strata2012), summarize,
                   Year = 2012,
                   Length = sum(LengthKM))
names(strat.2012) = c('Watershed', 'Category', 'Year', 'Strata.Length')
strat.2013 = ddply(subset(frames, Target2013=='Target'), .(CHaMPshed, Strata2013), summarize,
                   Year = 2013,
                   Length = sum(LengthKM))
names(strat.2013) = c('Watershed', 'Category', 'Year', 'Strata.Length')
...
var.decomp.data$Category = champ.sites.all$Strata2013[match(var.decomp.data$SiteName, champ.sites.all$Site_ID)]

# transform variance decomposition data into better format
# var.decomp.melt = melt(var.decomp.data, measure.vars=met.nms, id.vars=c('Site.Name', 'Year', 'Watershed', 'ValleyClass', 'Category.Name'), variable_name='Metric')
var.decomp.melt = melt(var.decomp.data, measure.vars=met.nms, id.vars=c('SiteName', 'Year', 'Watershed', 'ValleyClass', 'Category', 'Primary.Visit', 'CHaMP.10..Revisit'), variable_name='Metric')

# sites.df = merge(ddply(var.decomp.melt, .(Metric, Watershed, Category, Year), summarize, n.sites = length(unique(SiteName[!is.na(value)]))), subset(strat.all, Year==2013, c('Watershed', 'Category', 'Strata.Length')), all.x=T)
sites.df = merge(ddply(subset(var.decomp.melt, Primary.Visit=='Yes'), .(Metric, Watershed, Category, Year), summarize, n.sites = length(unique(SiteName[!is.na(value)]))), subset(strat.all, Year==2013, c('Watershed', 'Category', 'Strata.Length')), all.x=T)

sites.df$Adj.Wgt = with(sites.df, Strata.Length / n.sites)
sites.df$Adj.Wgt[sites.df$n.sites==0] = NA
# norm.wgt = ddply(sites.df, .(Metric, Watershed, Year), summarize,
  # n.sites = sum(n.sites),
  # TotalKM = sum(Strata.Length),
  # Norm.Wgt = Adj.Wgt / max(Adj.Wgt, na.rm=T))

# where and when are strata with missing strata length?
with(subset(sites.df, is.na(Strata.Length) & Metric=='Lgth_Bf'), table(Watershed, Year))
unique(subset(sites.df, is.na(Strata.Length), c('Watershed', 'Year', 'Category', 'n.sites')))
unique(subset(sites.df, is.na(Adj.Wgt), c('Watershed', 'Year', 'Category', 'n.sites')))


# add weights to variance decomposition melted dataframe
var.decomp.wgts = merge(var.decomp.melt, sites.df[,c('Watershed', 'Year', 'Category', 'Metric', 'Strata.Length', 'n.sites', 'Adj.Wgt')])

# re-adjust weights for re-visited sites
with(var.decomp.wgts, table(table(paste(SiteName, Year, Metric, sep='_'))))

visit.tab = ddply(var.decomp.wgts, .(SiteName, Year, Metric), summarize, n.visits = length(SiteName))
...
Var.Decomp = function(my.data, log.transf=T, rand.eff.all = c('Year', 'Watershed', 'ValleyClass', 'SiteName'), boot.wgts = T, n.boot=100, iterative.boot=T) {
  library(lme4)
  ...
  for(eff in rand.eff.all) if(length(unique(droplevels(subset(my.data, !is.na(mod.metric)))[,eff]))<=1) rand.eff = rand.eff[-match(eff, rand.eff)]
...
# Var.Decomp2 = function(my.data, log.transf=T, rand.eff.all = c('Year', 'Watershed', 'ValleyClass'), boot.wgts = T, n.boot=100, iterative.boot=T) {
#   library(lme4)
#   if(log.transf) my.data$mod.metric = log(my.data$value+0.1)
#   if(!log.transf) my.data$mod.metric = my.data$value
#
